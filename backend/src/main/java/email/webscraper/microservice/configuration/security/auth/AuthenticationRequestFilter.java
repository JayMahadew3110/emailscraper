package email.webscraper.microservice.configuration.security.auth;
...
public class AuthenticationRequestFilter extends OncePerRequestFilter {
    ...
    private void setupSpringSecurityContext(AccessToken accessToken) {
        boolean isAdmin = accessToken.isAdmin();
        Set<GrantedAuthority> authorities = new HashSet<>();
        if (isAdmin)
            authorities.add(new SimpleGrantedAuthority(SPRING_SECURITY_ROLE_PREFIX + "ADMIN"));
        else
            authorities.add(new SimpleGrantedAuthority(SPRING_SECURITY_ROLE_PREFIX+ "DEFAULT_USER"));
        UserDetails userDetails = new User(accessToken.getEmail(), "ignoredPassword", authorities);
